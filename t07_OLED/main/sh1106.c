...
void sh1106_init(sh1106_t *display) {
    ...
    // command according documentation in order
    i2c_master_write_byte(cmd, 0x00, true); // set lower column address
    i2c_master_write_byte(cmd, 0x10, true); // set higher column address
    i2c_master_write_byte(cmd, 0x30, true); // set pupm voltage value (i don't know how it is works)
    i2c_master_write_byte(cmd, 0x40, true); // set display start line
    i2c_master_write_byte(cmd, 0x81, true); // set contrast control mode
    i2c_master_write_byte(cmd, 0xA0, true); // set segment remap
    i2c_master_write_byte(cmd, 0xA4, true); // set entire display ON/OFF
    i2c_master_write_byte(cmd, 0xA6, true); // set normal/reverse display - means output info will HIGH or LOW color
    i2c_master_write_byte(cmd, 0xA8, true); // multiplex
    i2c_master_write_byte(cmd, 0xB0, true); // set page address (0xB0 - 0xB7)
    ...
    i2c_master_write_byte(cmd, 0xFF, true);
    i2c_master_write_byte(cmd, 0xAF, true); // alternatively turns the display on(0xAE) / off(0xAF)
    i2c_master_stop(cmd);
